...
async def test_agent_capabilities():
    ...
    with open("eval/basic_capabilities.test.json", "r") as f:
...
@pytest.mark.asyncio
@pytest.mark.xfail(reason="Agent asks for clarification even when Project ID is provided in prompt")
async def test_tool_selection():
    """Test the agent's tool selection capabilities."""
    with open("eval/tool_selection.test.json", "r") as f:
        eval_data = json.load(f)
...
async def test_metrics_analysis():
    ...
    with open("eval/metrics_analysis.test.json", "r") as f:
        eval_data = json.load(f)
