...
*   **Dynamic Tab Label**: The toolbar displays an icon and label that change based on the active `DashboardDataType` (Traces, Logs, Metrics, Alerts, Council, Charts). Each tab has a unique icon and accent color defined in `_tabConfig()`.
