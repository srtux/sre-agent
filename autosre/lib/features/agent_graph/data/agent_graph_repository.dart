...
AggregatedEdges AS (
  ...
  FROM GraphPaths GROUP BY source_id, target_id, source_type, target_type
),
BaseNodes AS (
  SELECT target_id AS id, target_type AS type, ANY_VALUE(COALESCE(agent_description, tool_description)) AS description,
    SUM(COALESCE(input_tokens, 0) + COALESCE(output_tokens, 0)) AS total_tokens, COUNTIF(status_code = 2) > 0 AS has_error
  FROM GraphPaths GROUP BY target_id, target_type
  UNION DISTINCT
  SELECT source_id AS id, source_type AS type, NULL AS description, 0 AS total_tokens, FALSE AS has_error
  FROM GraphPaths GROUP BY source_id, source_type
),
AggregatedNodes AS (
  SELECT bn.*,
