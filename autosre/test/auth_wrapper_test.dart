...
void main() {
  testWidgets(
    'AuthWrapper shows ConversationPage when authenticated (bypass mode)',
  ...
  testWidgets('AuthWrapper shows LoginPage when NOT authenticated', (
    WidgetTester tester,
  ) async {
    final mockAuth = MockAuthService(authenticated: false);

    await tester.pumpWidget(
      wrapWithProviders(const MaterialApp(home: AuthWrapper()), auth: mockAuth),
    );
