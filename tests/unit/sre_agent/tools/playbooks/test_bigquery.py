"""Tests for BigQuery playbook."""

from sre_agent.tools.playbooks.bigquery import get_playbook
...
class TestGetPlaybook:
    ...
    def test_has_valid_playbook_id(self) -> None:
        playbook = get_playbook()
        assert playbook.playbook_id == "bigquery-troubleshooting"

    def test_has_service_name(self) -> None:
        playbook = get_playbook()
        assert playbook.service_name == "BigQuery"

    def test_has_display_name(self) -> None:
        playbook = get_playbook()
        assert playbook.display_name == "BigQuery"
    ...
    def test_has_expected_issue_count(self) -> None:
        playbook = get_playbook()
        assert len(playbook.issues) == 3  # query_failed, slow_query, quota_exceeded
