"""Tests for Cloud SQL playbook."""

from sre_agent.tools.playbooks.cloud_sql import get_playbook
...
class TestGetPlaybook:
    ...
    def test_has_valid_playbook_id(self) -> None:
        playbook = get_playbook()
        assert playbook.playbook_id == "cloud-sql-troubleshooting"

    def test_has_service_name(self) -> None:
        playbook = get_playbook()
        assert playbook.service_name == "Cloud SQL"

    def test_has_display_name(self) -> None:
        playbook = get_playbook()
        assert playbook.display_name == "Cloud SQL"

    def test_has_valid_category(self) -> None:
        playbook = get_playbook()
        assert playbook.category is PlaybookCategory.DATA
    ...
    def test_has_expected_issue_count(self) -> None:
        playbook = get_playbook()
        assert (
            len(playbook.issues) == 3
        )  # connection_failed, high_latency, replication_lag
    ...
    def test_expected_issue_ids(self) -> None:
        ...
        expected = {
            ...
            "cloudsql-high-latency",
            "cloudsql-replication-lag",
        }
