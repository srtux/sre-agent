"""Tests for Pub/Sub playbook."""

from sre_agent.tools.playbooks.pubsub import get_playbook
...
class TestGetPlaybook:
    ...
    def test_has_valid_playbook_id(self) -> None:
        playbook = get_playbook()
        assert playbook.playbook_id == "pubsub-troubleshooting"

    def test_has_service_name(self) -> None:
        playbook = get_playbook()
        assert playbook.service_name == "Pub/Sub"

    def test_has_display_name(self) -> None:
        playbook = get_playbook()
        assert playbook.display_name == "Cloud Pub/Sub"

    def test_has_valid_category(self) -> None:
        playbook = get_playbook()
        assert playbook.category is PlaybookCategory.MESSAGING
    ...
    def test_has_expected_issue_count(self) -> None:
        playbook = get_playbook()
        assert len(playbook.issues) == 3  # not_delivered, high_latency, push_failures
    ...
    def test_expected_issue_ids(self) -> None:
        ...
        expected = {
            "pubsub-message-not-delivered",
            "pubsub-high-latency",
            "pubsub-push-failures",
        }
