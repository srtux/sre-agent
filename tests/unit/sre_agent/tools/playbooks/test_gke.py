"""Tests for GKE playbook."""

from sre_agent.tools.playbooks.gke import get_playbook
...
class TestGetPlaybook:
    ...
    def test_has_valid_playbook_id(self) -> None:
        playbook = get_playbook()
        assert playbook.playbook_id == "gke-troubleshooting"

    def test_has_service_name(self) -> None:
        playbook = get_playbook()
        assert playbook.service_name == "GKE"

    def test_has_display_name(self) -> None:
        playbook = get_playbook()
        assert playbook.display_name == "Google Kubernetes Engine"
    ...
    def test_has_expected_issue_count(self) -> None:
        playbook = get_playbook()
        assert (
            len(playbook.issues) == 6
        )  # image_pull, crashloop, unschedulable, node_not_ready, oom, hpa
    ...
    def test_has_general_diagnostic_steps(self) -> None:
        playbook = get_playbook()
        assert len(playbook.general_diagnostic_steps) == 4
    ...
    def test_expected_issue_ids(self) -> None:
        ...
        expected = {
            "gke-image-pull-backoff",
            "gke-crashloop-backoff",
            "gke-pod-unschedulable",
            "gke-node-not-ready",
            "gke-oom-killed",
            "gke-hpa-not-scaling",
        }
    ...
    def test_general_diagnostic_steps_have_tool_names(self) -> None:
        playbook = get_playbook()
        for step in playbook.general_diagnostic_steps:
            assert step.tool_name is not None
            assert step.tool_params is not None
