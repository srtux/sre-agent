...
def test_get_trace_client_default():
    from sre_agent.tools.clients.factory import GLOBAL_CONTEXT_CREDENTIALS

    with patch("google.cloud.trace_v1.TraceServiceClient") as mock_class:
        client = get_trace_client()
        assert client is not None
        mock_class.assert_called_once_with(credentials=GLOBAL_CONTEXT_CREDENTIALS)


def test_get_trace_client_with_context():
    mock_context = MagicMock()
    # When tool_context is provided, factory calls set_auth_context_from_tool_context
    # which uses GLOBAL_CONTEXT_CREDENTIALS eventually.
    with patch("google.cloud.trace_v1.TraceServiceClient") as mock_class:
...
def test_get_logging_client():
    ...
    with patch(
        "sre_agent.tools.clients.factory.LoggingServiceV2Client", create=True
    ) as mock_class:
