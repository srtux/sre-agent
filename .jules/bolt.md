...
## 2024-06-01 - [FastAPI Event Loop Blocking]
**Learning:** In `sre_agent/auth.py`, `google.oauth2.id_token.verify_oauth2_token` is synchronous and can block the async event loop in FastAPI while fetching public keys.
**Action:** Use `fastapi.concurrency.run_in_threadpool` to offload blocking synchronous calls like `verify_oauth2_token` to a thread pool to avoid blocking the event loop.
