...
## 2024-05-27 - [Parallelize `run_in_threadpool` Loops]
**Learning:** Sequential loops that offload blocking I/O calls via `fastapi.concurrency.run_in_threadpool` (e.g., querying monitoring data or multiple health checks in `get_application_health` and `get_application_metrics`) cause an N+1 latency bottleneck because each thread pool execution blocks the event loop's iteration awaiting the result.
**Action:** Always refactor sequential `await run_in_threadpool` calls inside loops into a list of tasks and execute them concurrently using `asyncio.gather(*tasks)` to significantly reduce aggregate latency.
