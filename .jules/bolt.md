...
## 2024-05-28 - [Eliminate N+1 latency in application telemetry]
**Learning:** Sequential loops of independent `run_in_threadpool` API and log queries (e.g., fetching metrics for multiple components in `sre_agent/tools/clients/app_telemetry.py`) cause severe N+1 latency bottlenecks.
**Action:** When making multiple independent async or offloaded sync calls, gather the tasks into a list and await them concurrently using `asyncio.gather` to eliminate N+1 latency bottlenecks.
