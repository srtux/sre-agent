...
## 2026-03-06 - Avoid statistics.mean for Performance
**Learning:** Python's statistics.mean is significantly slower (~30-60x) than calculating sum(list) / len(list) for small to moderate lists because it casts all numeric inputs to fractional type.
**Action:** Prefer sum()/len() (with empty list checks) in performance-critical loops.
