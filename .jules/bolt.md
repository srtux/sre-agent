...
## 2024-05-25 - [Parallelize Sequential Cloud Run and GKE queries]
**Learning:** Checking the application health linearly queried log errors for each Cloud Run and GKE cluster component, causing N+1 latency. We can optimize it by mapping tasks to elements in parallel.
**Action:** Always gather independent `run_in_threadpool` tasks inside sequential loops. Store mapping metadata and evaluate results utilizing `asyncio.gather(*tasks)` mapped with `zip(metadata, results, strict=True)` to preserve association and bypass N+1 blocking bottlenecks.
