...
## 2025-02-18 - [Avoid the `statistics` package for high-frequency operations]
**Learning:** Python's built-in `statistics` package (e.g., `statistics.mean`, `statistics.median`, `statistics.variance`, `statistics.stdev`) adds significant overhead due to internal exactness tracking. In latency/duration data processing where extreme precision is less critical than speed, this overhead becomes a bottleneck (benchmarked at ~13x slower).
**Action:** Replace `statistics` package functions with manual native calculations using `sum()` with generator expressions (e.g., `sum((x - mean) ** 2 for x in data) / (len(data) - 1)`), index lookups (for median), and `math.sqrt()` in performance-sensitive analysis tools.
