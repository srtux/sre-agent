...
## 2025-02-18 - Replacing Python statistics module with built-ins
**Learning:** Python's `statistics.mean` is ~40-80x slower than `sum(list) / len(list)` due to internal exactness tracking and fractional type casting. `statistics.median` on an already sorted list is ~240x slower than custom index-based logic (`list[len // 2]`). List comprehensions inside `sum()` are faster than generator expressions for variance/stdev.
**Action:** Prefer native built-ins in performance-critical statistical analysis functions, particularly when doing latency or duration analysis where exact rational precision is not needed.
