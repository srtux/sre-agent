...
## 2025-05-26 - [Replace statistics library with custom math functions]
**Learning:** Python's `statistics` module functions (e.g., `mean`, `variance`, `stdev`, `median`) are significantly slower than native math operations (e.g., `sum() / len()`, `math.sqrt()`). Replace them with native math equivalents for performance-critical data processing where minor precision loss is negligible.
**Action:** Created custom `math_utils.py` with optimized math functions and replaced imports across the application.
