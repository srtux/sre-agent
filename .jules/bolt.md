...
## 2025-02-18 - [Native Math vs Statistics Module]
**Learning:** Python's standard library `statistics` module (e.g., `mean`, `median`, `stdev`) is notoriously slow due to its overhead handling precision and different numeric types. Profiling shows that basic native math implementations (like `sum(x)/len(x)` and `math.sqrt()`) are roughly 8-10x faster for numerical data operations in latency-sensitive paths like trace processing.
**Action:** Replace `statistics` module usage with native math utility functions in performance-critical areas where minor precision loss from float arithmetic is negligible.
