...
## 2025-02-18 - Replacing `statistics` with Native Math
**Learning:** Python's `statistics.mean`, `statistics.median`, and `statistics.stdev` are significantly slower (e.g., ~10x slower) than manual calculations using `sum()` with list comprehensions and native `math` operations, because the `statistics` module tracks fractional exactness and arbitrary precision.
**Action:** Prefer using native math built-ins (e.g., `sum(data) / len(data)`, `math.sqrt()`, and manual index logic for median) for speed in latency/duration data processing where micro-precision loss is negligible.
