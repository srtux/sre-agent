...
## 2025-02-18 - [Native Math Operations for Better Performance]
**Learning:** Python's built-in `statistics` module (e.g., `mean`, `median`, `stdev`, `variance`) is significantly slower than native math alternatives (e.g., `sum() / len()`, `math.sqrt()`) because it performs additional safety checks, internal coercions to Decimal/Fraction, and error handling for empty lists. In our benchmarks, using native equivalents was approximately 4x faster.
**Action:** When aggregating list values for general performance statistics (like trace durations, latencies, or demo data generation) where slight precision loss is acceptable, always prefer native Python math operations to avoid the latency penalty of the `statistics` module.
