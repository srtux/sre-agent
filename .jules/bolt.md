...
## 2025-02-18 - [Parallelize Sequential Threads]
**Learning:** Performing multiple sequential `run_in_threadpool` operations inside a `for` loop across multiple resources (e.g. iterating over `metric_filters` or `cloud_run_services`) behaves like a blocking N+1 query pattern, resulting in higher latency.
**Action:** When performing independent `run_in_threadpool` tasks within a loop, accumulate the tasks in a list and invoke them concurrently using `asyncio.gather(*tasks)`, matching the results sequentially back to the resource filters. This eliminates latency bottlenecks and leverages full concurrent event-loop scaling.
