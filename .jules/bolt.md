...
**Learning:** The Python `statistics` module tracks exactness internally which makes it significantly slower (e.g., ~14x to 80x depending on the function) than native operations like `sum() / len()`, manual median index calculations, and `sum((x - mean) ** 2 for x in data) / (n - 1)` with `math.sqrt()`. This latency matters in data processing paths like trace analytics.
**Action:** Prefer native built-ins for statistical calculations in latency-sensitive paths where the slight precision differences are negligible. Always verify list lengths to handle odd/even elements for median and zero denominators.
