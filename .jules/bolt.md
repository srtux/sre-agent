...
## YYYY-MM-DD - Replace statistics with native math
**Learning:** Python's `statistics` module functions (like `mean`, `median`, `variance`, `stdev`) are significantly slower than native manual calculations using `sum()`, list indexing, and `math.sqrt()` due to exactness tracking and fractional type casting.
**Action:** Replaced `statistics` functions with native mathematical operations in latency/duration data processing where precision loss is negligible to improve performance.
