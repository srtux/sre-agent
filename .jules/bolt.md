...
## 2025-02-18 - [Replace Slow Statistics Module]
**Learning:** Python's `statistics` module functions (like `mean`, `median`, `stdev`, `variance`) are 40-80x slower than using native math built-ins (like `sum() / n` and `math.sqrt()`) because of exactness tracking. This causes substantial performance impact when calculating statistics on data arrays.
**Action:** Always prefer native implementations for arrays inside performance paths instead of relying on the standard `statistics` module unless exact mathematical precision is an absolute requirement that offsets the speed drop.
