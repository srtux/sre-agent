...
## 2024-05-31 - Fast Math Operations in Statistical Analysis
**Learning:** Python's `statistics.mean`, `statistics.median`, `statistics.variance` and `statistics.stdev` are significantly slower than custom native arithmetic using `sum()` and math operations (e.g. ~40-80x slower for mean, ~240x slower for median, ~7-8x for stdev/variance) due to internal precision tracking.
**Action:** Replace `statistics` built-ins with manual calculations like `sum(x) / len(x)`, direct indexing for median, and `sum([(x - mean)**2 for x in data]) / (n-1)` in performance-critical areas like `statistical_analysis.py` to boost performance. Always assign list slices to local variables before using them in manual calculations.
