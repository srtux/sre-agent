...
## 2025-02-18 - `statistics.mean` Performance Bottleneck
**Learning:** Python's built-in `statistics.mean` is surprisingly slow because it prioritizes numeric stability and uses `fractions.Fraction` internally for exact calculations. In hot paths like trace processing, micro-benchmarks show `sum(data) / len(data)` is over 40x faster than `statistics.mean(data)` for lists of floats.
**Action:** Replace `statistics.mean(data)` with `sum(data) / len(data) if data else 0.0` in performance-critical areas such as statistical analysis algorithms, anomaly detection, and synthetic trace generation to improve latency.
