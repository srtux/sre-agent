...
**Learning:** Functions like `get_application_health` and `get_application_metrics` loop over components and perform independent API calls using `run_in_threadpool`. Performing these sequentially leads to O(N) wait times (where N is component count).
**Action:** Append the task coroutines (e.g., `run_in_threadpool`) to a list and use `asyncio.gather(*tasks)` to execute them concurrently. Use `zip(metadata, results, strict=True)` to map the results back to their originating component.
