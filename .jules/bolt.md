...
## 2025-04-16 - Fast Statistical Calculations
**Learning:** Python's standard `statistics` module is surprisingly slow due to internal exactness tracking and object overhead. `statistics.mean` is ~75x slower than `sum()/len()`, and `statistics.variance` is ~6x slower than manual calculation `sum((x-mean)**2)/(n-1)`. For data latency and metrics where extreme precision loss is negligible, standard library functions are a bottleneck.
**Action:** Prefer native Python built-ins like `sum()` and generator expressions over `statistics` functions for high-frequency or latency-sensitive endpoints.
