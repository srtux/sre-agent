...
REMEDIATION_PATTERNS: dict[str, Any] = {
    # Memory issues
    "oom_killed": {
...
def get_gcloud_commands(
    ...
    **kwargs: Any,
    ...
    Returns:
        Dictionary with gcloud commands and explanations.
...
def find_similar_past_incidents(
    ...
    try:
        ...
        for incident in known_incidents:
            if str(incident["pattern"]) in pattern_lower or pattern_lower in str(
                incident["pattern"]
            ):
                if (
                    service_name is None
                    or str(service_name).lower() in str(incident["service"]).lower()
                ):
