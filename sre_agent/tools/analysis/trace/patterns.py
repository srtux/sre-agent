...
def _get_start_time(span: dict[str, Any]) -> float | None:
    """Get start time in milliseconds, preferring pre-calculated unix timestamp."""
    if "start_time_unix" in span:
        return float(span["start_time_unix"]) * 1000
    return _parse_timestamp(span.get("start_time", ""))


def _get_end_time(span: dict[str, Any]) -> float | None:
    """Get end time in milliseconds, preferring pre-calculated unix timestamp."""
    if "end_time_unix" in span:
        return float(span["end_time_unix"]) * 1000
    return _parse_timestamp(span.get("end_time", ""))
...
def _get_span_duration(span: dict[str, Any]) -> float | None:
    ...
    start = _get_start_time(span)
    end = _get_end_time(span)
...
def _detect_retry_storm_impl(
    ...
    for name, span_list in spans_by_name.items():
        ...
        if len(span_list) >= threshold or is_retry_span:
            ...
            sorted_spans = sorted(
                span_list,
                key=lambda s: _get_start_time(s) or 0,
            )

            # Check for sequential pattern (small gaps between spans)
            sequential_count = 1
            for i in range(1, len(sorted_spans)):
                prev_end = _get_end_time(sorted_spans[i - 1])
                curr_start = _get_start_time(sorted_spans[i])
...
def detect_retry_storm(
    ...
    try:
        ...
        result = _detect_retry_storm_impl(trace, threshold)
...
def _detect_cascading_timeout_impl(
    trace: dict[str, Any], timeout_threshold_ms: float = 1000
) -> dict[str, Any]:
    """Internal implementation of cascading timeout detection logic."""
    spans = trace.get("spans", [])
    timeout_spans = []
    ...
    for c in sorted(cascade_chains, key=lambda ch: ch["chain_length"], reverse=True):
        affected = set(c["affected_spans"])
        is_subset = any(affected <= set(uc["affected_spans"]) for uc in unique_chains)
        if not is_subset:
...
def detect_cascading_timeout(
    ...
    try:
        ...
        result = _detect_cascading_timeout_impl(trace, timeout_threshold_ms)
...
def _detect_connection_pool_issues_impl(
    trace: dict[str, Any], wait_threshold_ms: float = 100
) -> dict[str, Any]:
    """Internal implementation of connection pool issue detection."""
    spans = trace.get("spans", [])
    pool_issues = []
    ...
    for s in spans:
        ...
        if duration >= wait_threshold_ms:
            ...
            waiting = labels.get("pool.waiting") or labels.get("db.waiting_requests")
...
def detect_connection_pool_issues(
    ...
    try:
        ...
        result = _detect_connection_pool_issues_impl(trace, wait_threshold_ms)
...
def detect_all_sre_patterns(
    ...
    try:
        # Optimization: Fetch trace once and reuse for all detectors
        trace = fetch_trace_data(trace_id, project_id, tool_context=tool_context)
        if "error" in trace:
            return BaseToolResponse(status=ToolStatus.ERROR, error=trace["error"])
        ...
        retry_res = _detect_retry_storm_impl(trace)
        if retry_res.get("has_retry_storm"):
            results["patterns"].extend(retry_res["retry_patterns"])
        ...
        timeout_res = _detect_cascading_timeout_impl(trace)
        if timeout_res.get("cascade_detected"):
            results["patterns"].append(
                {
                    "pattern_type": "cascading_timeout",
                    "chains": timeout_res["cascade_chains"],
                    "impact": timeout_res["impact"],
                }
            )
            results["recommendations"].append(
                {
        ...
        pool_res = _detect_connection_pool_issues_impl(trace)
        if pool_res.get("has_pool_exhaustion"):
            results["patterns"].append(
                {
                    "pattern_type": "connection_pool_exhaustion",
                    "issues": pool_res["pool_issues"],
                    "total_wait_ms": pool_res["total_wait_ms"],
                }
            )
            results["recommendations"].append(
                {
