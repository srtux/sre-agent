...
def _validate_trace_quality_impl(trace: TraceData) -> dict[str, Any]:
    ...
    for span in spans:
        ...
        try:
            ...
            if s_start_unix is not None and s_end_unix is not None:
                # Optimized path: Use float timestamps
                ...
                if parent_id and parent_id in span_map:
                    ...
                    if p_start_unix is not None and p_end_unix is not None:
                        # Allow small floating point epsilon for clock skew checks
                        if (
                            s_start_unix < p_start_unix - 1e-6
                            or s_end_unix > p_end_unix + 1e-6
                        ):
                            issues.append(
                                {
                                    "type": "clock_skew",
