    ...
    for span in spans:
        ...
        try:
            ...
            if start_unix is not None and end_unix is not None:
                # Optimized path: Use unix timestamps (float)
                ...
                if parent_id and parent_id in span_map:
                    ...
                    if p_start_unix is not None and p_end_unix is not None:
                        ...
                        # Fallback if parent missing unix timestamp
                ...
                if start_str and end_str:
                    ...
                    if parent_id and parent_id in span_map:
                        ...
                        if p_start_str and p_end_str:
                            p_start_dt = datetime.fromisoformat(
                                p_start_str.replace("Z", "+00:00")
                            )
                            p_end_dt = datetime.fromisoformat(
                                p_end_str.replace("Z", "+00:00")
                            )

                            if start < p_start_dt or end > p_end_dt:
                                issues.append(
                                    {
                                        "type": "clock_skew",
                                        "span_id": span_id,
