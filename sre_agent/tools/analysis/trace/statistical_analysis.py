...
def _compute_latency_statistics_impl(
    ...
    if count > 1:
        variance_latency = sum([(x - mean_latency) ** 2 for x in latencies]) / (
            count - 1
        )
        stats["stdev"] = math.sqrt(variance_latency)
        stats["variance"] = variance_latency
    else:
        stats["stdev"] = 0
        stats["variance"] = 0
...
def analyze_trace_patterns(
    ...
    for name, perf in span_performance.items():
        ...
        mean_dur = sum(durs) / c
        if c > 1:
            var_dur = sum([(x - mean_dur) ** 2 for x in durs]) / (c - 1)
