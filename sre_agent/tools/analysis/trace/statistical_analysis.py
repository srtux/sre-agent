...
def _compute_latency_statistics_impl(
    ...
    latencies.sort()
    count = len(latencies)
    mean_val = sum(latencies) / count if count > 0 else 0
    median_val = (
        latencies[count // 2]
        if count % 2 != 0
    ...
    for name, durs in span_durations.items():
        ...
        durs.sort()
        c = len(durs)
        span_mean = sum(durs) / c if c > 0 else 0
        per_span_stats[name] = {
            "count": c,
            "mean": span_mean,
            "min": durs[0],
        ...
        if c > 1:
            var = sum((x - span_mean) ** 2 for x in durs) / (c - 1)
            per_span_stats[name]["variance"] = var
            per_span_stats[name]["stdev"] = math.sqrt(var)
        else:
...
def analyze_trace_patterns(
    ...
    for name, perf in span_performance.items():
        if perf["occurrences"] < 2:
            continue
        durs = perf["durations"]
        mean_dur = sum(durs) / len(durs) if len(durs) > 0 else 0
        if len(durs) > 1:
            var = sum((x - mean_dur) ** 2 for x in durs) / (len(durs) - 1)
            stdev_dur: float = math.sqrt(var)
        else:
            stdev_dur = 0.0
