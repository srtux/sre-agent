...
def _compute_latency_statistics_impl(
    ...
    for name, durs in span_durations.items():
        ...
        if c > 1:
            variance_val = sum((x - span_mean) ** 2 for x in durs) / (c - 1)
            per_span_stats[name]["stdev"] = math.sqrt(variance_val)
            per_span_stats[name]["variance"] = variance_val
        else:
...
def analyze_trace_patterns(
    ...
    for name, perf in span_performance.items():
        ...
        mean_dur = sum(durs) / c
        if c > 1:
            variance_val = sum((x - mean_dur) ** 2 for x in durs) / (c - 1)
            stdev_dur = math.sqrt(variance_val)
        else:
            stdev_dur = 0.0
