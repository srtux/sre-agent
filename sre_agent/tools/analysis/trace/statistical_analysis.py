...
def _compute_latency_statistics_impl(
    ...
    if count > 1:
        stats["stdev"] = fast_stdev(latencies)
        stats["variance"] = fast_variance(latencies)
    else:
        stats["stdev"] = 0
        stats["variance"] = 0
    ...
    for name, durs in span_durations.items():
        ...
        if c > 1:
            per_span_stats[name]["stdev"] = fast_stdev(durs)
            per_span_stats[name]["variance"] = fast_variance(durs)
        else:
...
def perform_causal_analysis(
    ...
    for span_id, target_span in target_spans_by_id.items():
        ...
        if not baseline_durations:
            continue
        baseline_avg = fast_mean(baseline_durations)
...
def analyze_trace_patterns(
    ...
    for name, perf in span_performance.items():
        if perf["occurrences"] < 2:
            continue
        durs = perf["durations"]
        mean_dur = fast_mean(durs)
        stdev_dur: float = fast_stdev(durs) if len(durs) > 1 else 0.0
    ...
    if len(trace_durations) >= 3:
        first = fast_mean(trace_durations[: len(trace_durations) // 2])
        second = fast_mean(trace_durations[len(trace_durations) // 2 :])
