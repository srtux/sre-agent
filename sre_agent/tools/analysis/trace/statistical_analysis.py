...
def _get_span_duration_ms(span: dict[str, Any]) -> float | None:
    """Calculates span duration in milliseconds using unix timestamps if available."""
    # 1. Trust explicit duration if present
    duration = span.get("duration_ms")
    if duration is not None:
        return float(duration)

    # 2. Optimized path: Use unix timestamps (floats)
...
def compute_service_level_stats(
    ...
    for t_data in traces_data:
        for s in t_data.get("spans", []):
            ...
            dur = 0.0
            span_dur = _get_span_duration_ms(s)
