...
def _compute_latency_statistics_impl(
    ...
    # Optimization: Native sum() and math operations are significantly faster
    # than the statistics module due to internal precision tracking overhead.
    ...
    if count > 1:
        # Optimization: Manual sample variance calculation is faster
...
def perform_causal_analysis(
    ...
    for span_id, target_span in target_spans_by_id.items():
        ...
        if not baseline_durations:
            continue
        # Optimization: Native sum() is faster than statistics.mean()
...
def analyze_trace_patterns(
    ...
    for name, perf in span_performance.items():
        ...
        durs = perf["durations"]
        c = len(durs)
        # Optimization: Fast native math instead of statistics module
        mean_dur = sum(durs) / c
        if c > 1:
            span_var = sum([(x - mean_dur) ** 2 for x in durs]) / (c - 1)
            stdev_dur = math.sqrt(span_var)
        else:
            stdev_dur = 0.0
