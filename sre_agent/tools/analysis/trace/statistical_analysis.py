...
def _compute_latency_statistics_impl(
    ...
    if count > 1:
        ...
        stats["stdev"] = math.sqrt(
            sum((x - mean_val) ** 2 for x in latencies) / (count - 1)
        )
    ...
    for name, durs in span_durations.items():
        ...
        if c > 1:
            per_span_stats[name]["stdev"] = math.sqrt(
                sum((x - span_mean) ** 2 for x in durs) / (c - 1)
            )
...
def analyze_trace_patterns(
    ...
    for name, perf in span_performance.items():
        ...
        stdev_dur: float = (
            math.sqrt(sum((x - mean_dur) ** 2 for x in durs) / (n_durs - 1))
            if n_durs > 1
            else 0.0
        )
