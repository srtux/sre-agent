...
def _compute_latency_statistics_impl(
    ...
    if count > 1:
        v = sum((x - stats["mean"]) ** 2 for x in latencies) / (count - 1)
        stats["stdev"] = math.sqrt(v)
        stats["variance"] = v
    else:
        stats["stdev"] = 0
        stats["variance"] = 0
    ...
    for name, durs in span_durations.items():
        ...
        if c > 1:
            v = sum((x - span_mean) ** 2 for x in durs) / (c - 1)
            per_span_stats[name]["stdev"] = math.sqrt(v)
            per_span_stats[name]["variance"] = v
        else:
...
def analyze_trace_patterns(
    ...
    if len(trace_durations) >= 3:
        ...
        first_half = trace_durations[:mid]
        second_half = trace_durations[mid:]
