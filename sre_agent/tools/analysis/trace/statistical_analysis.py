...
def _get_span_duration(span: dict[str, Any]) -> float | None:
    """Helper to calculate span duration, prioritizing pre-calculated unix timestamps."""
    ...
    if start_str and end_str:
        return _parse_timestamp(start_str, end_str)
...
def _parse_timestamp(start_str: str, end_str: str) -> float | None:
    ...
    try:
        # Use .replace('Z', '+00:00') for python < 3.11 compatibility with Z
...
def _analyze_critical_path_impl(trace_data: dict[str, Any]) -> dict[str, Any]:
    ...
    parsed_spans = {}
    for s in spans:
        # Optimized timestamp parsing using pre-calculated values
        start = s.get("start_time_unix")
        end = s.get("end_time_unix")
