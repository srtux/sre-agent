...
def analyze_trace_patterns(
    ...
    if len(trace_durations) >= 3:
        first_durs = trace_durations[: len(trace_durations) // 2]
        first = sum(first_durs) / len(first_durs) if first_durs else 0.0
        second_durs = trace_durations[len(trace_durations) // 2 :]
        second = sum(second_durs) / len(second_durs) if second_durs else 0.0
