...
def _compute_latency_statistics_impl(
    ...
    latencies.sort()
    count = len(latencies)
    latencies_mean = sum(latencies) / count
    ...
    if count > 1:
        variance = sum([(x - latencies_mean) ** 2 for x in latencies]) / (count - 1)
...
def analyze_trace_patterns(
    ...
    if len(trace_durations) >= 3:
        half = trace_durations[: len(trace_durations) // 2]
        first = sum(half) / len(half)
