...
def _get_ts_val(ts_proto: Any) -> float:
    """Helper to extract timestamp value from proto."""
    ...
    try:
        ...
        for trace in response:
            ...
            if trace.spans:
                ...
                summary["start_time"] = _get_ts_str(root_span.start_time)
    ...
    try:
        ...
        def _calculate_example_traces() -> dict[str, Any]:
            ...
            p50 = (
                latencies[c // 2]
                if c % 2 != 0
                else (latencies[c // 2 - 1] + latencies[c // 2]) / 2
                if c > 0
                else 0
            )
            mean = sum(latencies) / c if c > 0 else 0
            if c > 1:
                variance_val = sum((x - mean) ** 2 for x in latencies) / (c - 1)
                stdev = math.sqrt(variance_val)
            else:
                stdev = 0
