...
class DemoDataGenerator:
    ...
    def get_topology(
        ...
        for nid, ns in node_stats.items():
            avg_dur = fast_mean(ns["durations"]) if ns["durations"] else 0.0
            nodes.append(
                {
                    "id": nid,
                    "type": ns["type"],
        ...
        for (src, tgt), es in edge_stats.items():
            avg_dur = fast_mean(es["durations"]) if es["durations"] else 0.0
            edges.append(
                {
                    "id": f"{src}->{tgt}",
                    "source": src,
    ...
    def get_edge_detail(
        ...
        return {
            ...
            "errorCount": error_count,
            "errorRate": round(error_rate, 4),
            "avgDurationMs": round(fast_mean(durations), 2) if durations else 0.0,
    ...
    def get_timeseries(
        ...
        for node_id, buckets in per_node.items():
            ...
            while t < _END_TIME:
                ...
                avg_dur = fast_mean(durations) if durations else 0.0
                points.append(
                    {
                        "bucket": key,
                        "callCount": len(spans),
    ...
    def get_dashboard_kpis(
        ...
        avg_turns = fast_mean([s["turns"] for s in sessions]) if sessions else 0.0
        ...
        prev_avg_turns = (
            fast_mean(prev_session_turns.values()) if prev_session_turns else 0.0
        )
            ...
            agent_sessions.append(
                {
                    ...
                    "errorCount": error_count,
                    "avgLatencyMs": round(fast_mean(latencies), 2)
                    if latencies
                    else 0.0,
            ...
            tools_list.append(
                {
                    ...
                    "errorRate": round(err_rate, 4),
                    "avgDurationMs": round(fast_mean(ts["durations"]), 2)
                    if ts["durations"]
                    else 0.0,
